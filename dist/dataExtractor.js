...
var generateLine = function (line) { return ({
...
function dataExtractor(json) {
    ...
    var result = {
        invoicer: generateCompany(json.FatturaElettronica.FatturaElettronicaHeader.CedentePrestatore),
        invoicee: generateCompany(json.FatturaElettronica.FatturaElettronicaHeader.CessionarioCommittente),
        installments: body.map(function (installmentJson) {
            ...
            var returnedInstallment = {
                ...
                totalAmount: generalData.ImportoTotaleDocumento() || payment.ImportoPagamento(0),
                issueDate: new Date(generalData.Data(new Date())),
                description: generalData.Causale(),
                lines: Array.isArray(lines)
                    ? lines.map(function (line) { return generateLine(line); })
                    : [lines].map(function (line) { return generateLine(line); }),
                taxSummary: {
            ...
            if (attachments) {
                returnedInstallment.attachments = Array.isArray(attachments)
                    ? attachments.map(function (attachment) { return ({
